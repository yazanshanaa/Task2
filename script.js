...
const toggleTask = id => { tasks = tasks.map(t=>t.id===id?{...t,done:!t.done}:t); save(); render(); };
const deleteTask = id => { tasks = tasks.filter(t=>t.id!==id); save(); render(); };
const deleteDone = () => { tasks = tasks.filter(t=>!t.done); save(); render(); };
